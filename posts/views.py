...
def Delete_Message(request, slug):
    obj = InternalMessage.objects.get(id=request.POST['id_ref'])
    if request.method == 'POST':
        obj.delete()
        print("h e l l o   w o r l d")

    return redirect(str(request.path)[:13])
...
def Detail(request, slug):
    ...
    if request.method == 'POST':
        ...
        message.accom = accom
        if request.user == accom.user:
            message.direction = False
        else:
            message.direction = True
        message.viewer = request.user
        message.message = request.POST['message_box']
        message.save()
